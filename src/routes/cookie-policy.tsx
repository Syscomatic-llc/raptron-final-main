...
const STRICTLY_NECESSARY: CookieRow[] = [
  {
    name: "session_id",
    provider: "raptron.com",
    purpose: "Maintains your authenticated session state.",
    duration: "Session",
  },
  {
    name: "csrf_token",
    provider: "raptron.com",
    purpose: "Cross-site request forgery protection for form submissions.",
    duration: "Session",
  },
  {
    name: "cookie_consent",
...
const ANALYTICS_COOKIES: CookieRow[] = [
  {
    ...
    purpose:
      "Distinguishes unique website users for aggregate traffic analysis.",
    duration: "24 months",
  },
  {
    name: "_ga_*",
    provider: "Google Analytics",
    purpose: "Maintains session state for Google Analytics 4 (GA4).",
    duration: "24 months",
  },
  {
    name: "_gid",
    provider: "Google Analytics",
    purpose: "Distinguishes users for session-level reporting.",
    duration: "24 hours",
  },
  {
    name: "_gat",
...
const FUNCTIONAL_COOKIES: CookieRow[] = [
  {
    name: "locale_pref",
    provider: "raptron.com",
    purpose: "Remembers your language or region preference.",
    duration: "12 months",
  },
  {
    name: "ui_theme",
    provider: "raptron.com",
    purpose: "Stores display preference settings.",
    duration: "12 months",
  },
];
...
const MARKETING_COOKIES: CookieRow[] = [
  {
    ...
    purpose: "Used by Meta to deliver and measure advertising effectiveness.",
    duration: "3 months",
  },
  {
    name: "li_fat_id",
    provider: "LinkedIn",
    purpose: "Tracks conversions from LinkedIn advertising campaigns.",
    duration: "30 days",
  },
  {
    name: "_gcl_au",
    provider: "Google Ads",
    purpose: "Stores and tracks ad conversion information from Google Ads.",
    duration: "3 months",
  },
];
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-surface-tinted">
            <th className="text-left font-semibold text-ink px-4 py-2.5 border border-hairline">
              Cookie Name
            </th>
            <th className="text-left font-semibold text-ink px-4 py-2.5 border border-hairline">
              Provider
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr
              key={i}
              className={i % 2 === 0 ? "bg-white" : "bg-surface-tinted/30"}
            >
              <td className="px-4 py-2.5 border border-hairline font-mono text-xs text-ink/80">
                {r.name}
              </td>
              ...
              <td className="px-4 py-2.5 border border-hairline text-ink/65 whitespace-nowrap">
                {r.duration}
              </td>
            </tr>
          ))}
...
function ConsentPanel() {
  const [prefs, setPrefs] = useState<CookieConsent>(() => {
    return getConsent() || PREFS_DEFAULT;
  });
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    const handleUpdate = () => {
      setPrefs(getConsent() || PREFS_DEFAULT);
    };
    ...
    <div className="rounded-2xl border-2 border-brand/20 bg-white shadow-card p-6 lg:p-8 my-8">
      ...
      <div className="flex items-start justify-between gap-4 py-4 border-b border-hairline">
        <div>
          ...
          <p className="text-xs text-ink/55 mt-0.5 max-w-md">
            Required for the website to function. They cannot be disabled - no
            personal data is collected beyond what is essential for operation.
          </p>
        </div>
        <div className="shrink-0 px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-semibold">
          Always on
        </div>
      </div>
      ...
      {categories.map((c) => (
        <div
          key={c.key}
          className="flex items-start justify-between gap-4 py-4 border-b border-hairline last:border-0"
        >
          <div>
          ...
          >
            <span
              className={`inline-block size-4 rounded-full bg-white shadow transition-transform duration-300 ${prefs[c.key] ? "translate-x-6" : "translate-x-1"}`}
            />
          </button>
        </div>
      ))}
      ...
      <div className="flex flex-wrap gap-3 mt-6">
        <button
          onClick={save}
          className="inline-flex items-center gap-2 h-10 px-6 rounded-full bg-brand text-white text-sm font-semibold hover:bg-brand-deep transition-colors cursor-pointer"
        >
        ...
        <button
          ...
          className="inline-flex items-center h-10 px-6 rounded-full border border-hairline text-sm font-semibold text-ink/70 hover:border-ink/30 hover:text-ink transition-colors cursor-pointer"
        >
          Reject all optional
        </button>
        <button
      ...
      <section className="relative bg-ink text-white overflow-hidden pt-36 pb-20">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <p className="text-white/60 text-lg max-w-2xl leading-relaxed mb-6">
            This Cookie Policy explains how RAPTRON Digital Solutions LLC uses
            cookies and similar tracking technologies on its website, in
            accordance with the UAE PDPL and applicable data protection law.
          </p>
        ...
        <div className="max-w-4xl">
          <S id="c1" title="What Are Cookies?" n={1}>
            <p>
              ...
              analytical information to website owners.
            </p>
            <p>
              We also use similar technologies such as{" "}
              <strong>web beacons</strong>, <strong>pixel tags</strong>, and{" "}
              <strong>local storage</strong> that function in a comparable way
              to cookies. References to &ldquo;cookies&rdquo; in this Policy
              encompass all such technologies unless otherwise stated.
            </p>
          </S>
          ...
          <S id="c2" title="How We Use Cookies" n={2}>
            ...
            <ul>
              ...
              <li>
                Understand how visitors interact with the website through
            ...
            <p>
              We do <strong>not</strong> use cookies to build individual
              profiles for automated decision-making, to target advertising
              based on sensitive personal characteristics, or to sell data to
              third parties.
            </p>
          </S>
          ...
          <S id="c3" title="Categories of Cookies We Use" n={3}>
            <h3 className="font-display font-bold text-ink text-base mt-4 mb-2 flex items-center gap-2">
              <span className="size-5 rounded bg-emerald-100 text-emerald-700 text-[10px] font-bold flex items-center justify-center">
                A
              </span>
              Strictly Necessary
            </h3>
            <p>
              Essential for the operation of the website. They enable core
              security functions, session management, and form submissions.
              These cookies cannot be disabled through our preference centre.
            </p>
            ...
            <h3 className="font-display font-bold text-ink text-base mt-6 mb-2 flex items-center gap-2">
              <span className="size-5 rounded bg-blue-100 text-blue-700 text-[10px] font-bold flex items-center justify-center">
                B
              </span>
            ...
            <p>
              ...
              collecting anonymised, aggregate information. No data is shared
              with Google in a way that identifies you personally under our
              implementation.
            </p>
            <CookieTable rows={ANALYTICS_COOKIES} />

            <h3 className="font-display font-bold text-ink text-base mt-6 mb-2 flex items-center gap-2">
              <span className="size-5 rounded bg-purple-100 text-purple-700 text-[10px] font-bold flex items-center justify-center">
                C
              </span>
              Functional
            </h3>
            <p>
              Enable enhanced functionality and personalisation. Disabling these
              may affect your experience but will not prevent core website use.
            </p>
            <CookieTable rows={FUNCTIONAL_COOKIES} />

            <h3 className="font-display font-bold text-ink text-base mt-6 mb-2 flex items-center gap-2">
              <span className="size-5 rounded bg-amber-100 text-amber-700 text-[10px] font-bold flex items-center justify-center">
                D
              </span>
              Marketing &amp; Advertising
            </h3>
            <p>
              Used to measure the effectiveness of advertising campaigns on
              third-party platforms. These cookies are only set with your
              explicit consent.
            </p>
          ...
          <S id="c4" title="Your Cookie Preferences" n={4}>
            <p>
              ...
              preference centre below. Note that disabling certain cookies may
          ...
          <S id="c5" title="Browser-Level Controls" n={5}>
            <p>
              In addition to our preference centre, you can control and delete
              cookies through your browser settings. Most modern browsers allow
              you to:
            </p>
            <ul>
              <li>
                View all cookies stored on your device and delete them
            ...
            <p>
              Note that if you block all cookies, you may not be able to access
              all or parts of our website. For browser-specific instructions:
            </p>
            <ul>
              <li>
                <a
          ...
          <S id="c6" title="Opt-Out of Analytics" n={6}>
            ...
            <p>
              For opt-out from interest-based advertising more broadly, you may
              use:
            </p>
            <ul>
              <li>
          ...
          <S id="c7" title="Updates to This Policy" n={7}>
            <p>
              ...
              changes in the cookies we use, legal requirements, or our privacy
              practices. Material updates will be communicated by posting a
              revised Policy with an updated effective date. We may also display
              a cookie consent notice on the website if the nature of the
              cookies we use changes materially.
            </p>
          </S>
